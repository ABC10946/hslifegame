...
drawField fieldS = pictures (map (drawCell fieldS) posMat)
    where
        posMat = [(x,y) | x <- [0,1..(length (head (field fieldS))-1)], y <- [0,1..(length (field fieldS) - 1)]]
...
drawCell fieldS (x_pos,y_pos) = translate (x_base + fX_pos * fCellSize) (y_base - fY_pos * fCellSize) rectPict
    where
        ...
        rectPict 
            |cState == (True,Live) = color magenta $ rectangleSolid fCellSize fCellSize
            |fst cState            = color green $ rectangleSolid fCellSize fCellSize
